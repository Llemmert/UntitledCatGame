...
public:
	MyGame(int w = 840, int h = 480) : Game("UntitledCat", w, h)
	{
		...
		sound = media->readWav("media/Mystical_Fountain.wav");
		...
		for (int i = 0; i < 2; i++) // loops twice, once for each cat
		{
			if (i == 0) // top cat y position; i=0 in loop so this is the flag for cat1 initiation
			{
				...
				cats.push_back(new Cat(ren, &a, &src, w / 2, yaxis, 0, 400, 0, 1000));
			...
			else // bottom cat y position; i!=0 in loop so this is the flag for cat2 initiation
			{
				...
				SDL_QueryTexture(a.getTexture(), NULL, NULL, &src.w, &src.h);
				// cat is 40 by 32 pixels
				cats.push_back(new Cat(ren, &a2, &src, w / 2, yaxis, 0, -400, 0, -1000));
	...
	void update(double dt)
	{
		...
		// cout << cats[0]->isGrounded() << endl;

		// src.x = cats[0]->getx() - 650 / 2;
		// src.y = cats[0]->gety() - 480 / 2;
		// if(src.x < 0)
		// 	src.x = 0;
		// if(src.y < 0)
		// 	src.y = 0;
		// src.x = cats[0]->getx();
		// src.y = cats[0]->gety();
		SDL_RenderCopy(ren, b.getTexture(), &src, NULL);
